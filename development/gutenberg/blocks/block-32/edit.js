...
import blockImage from '../../../../admin/assets/img/blocks/block-32.jpg';
      ...
      <div {...blockProps}>
        <div className="advanced-block">
          <div className="block-info">
            <span className="block-info-title">🎨 Block 32 - Видео (слайдер)</span>
            <ToggleControl
          ...
          {isPreview && (
            <div className="advanced-block-content">
              <div className="repeater-items numerations-items numerations-items-03">
                {items.map((item, index) => (
                  <div key={index} className="repeater-item numerations-item">
                    ...
                    {itemsList.renderBlockThirtyTwo(item, index)}
                  </div>
                ))}
              </div>
              <Button
                onClick={() => itemsList.add({ image: '', shadow: '', link: '' })}
                className="add-repeater-item"
                style={{ position: 'relative', display: 'block', width: '100%', textAlign: 'center', border: '1px solid rgba(0, 124, 186, 0.5)', zIndex: '20' }}
              >
