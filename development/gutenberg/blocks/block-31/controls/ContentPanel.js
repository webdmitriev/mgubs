...
const ContentPanel = ({ attributes, setAttributes }) => {
  const { teacherDescr } = attributes;

  const { typographField, typographAllFields } = useTypograf(attributes, setAttributes, ['teacherDescr']);

  const hasTextToTypograph = teacherDescr
